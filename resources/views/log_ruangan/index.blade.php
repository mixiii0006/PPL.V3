                        ...
                        <tbody>
                            ...
                            @else
                                @foreach ($r->jadwalRuangans->sortBy('pemetaan.jam_mulai') as $jadwal)
                                    ...
                                    {{-- <x-hapus :id="$jadwal->id" :route="route('log_ruangan.destroy', $jadwal->pemetaan->id)"/>
                                        ...
                                        <div id="{{$jadwal->id}}-updateProductModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden   bg-gray-500 bg-opacity-25 fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                                ...
                                                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                    ...
                                                    <!-- Modal body -->
                                                    <form action="{{ route('log_ruangan.update', $jadwal->pemetaan->id) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                ...
                                                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                    ...
                                                    <form action="{{ route('log_ruangan.update', $jadwal->id) }}" method="POST">
                                                        ...
                                                        <div class="grid gap-4 mb-7 sm:grid-cols-1">
                                                            ...
                                                            <label for="nama_matakuliah-{{$jadwal->id}}" class="block  text-sm font-medium text-gray-900 dark:text-white">Mata Kuliah</label>
                                                            <select name="matakuliah_id" id="nama_matakuliah-{{$jadwal->id}}" class="js-example-basic-single bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" name="state"  >

                                                                @foreach ($matakuliah as $item)
                                                                    <option value="{{  $item->id }}">{{ old('nama_matakuliah', $item->nama_matakuliah) }}</option>
                                                                @endforeach
                                                            </select>
                                                            <label for="Nama-{{$jadwal->id}}" class="block  text-sm font-medium text-gray-900 dark:text-white">Dosen</label>
                                                            <select name="dosen_id" id="Nama-{{$jadwal->id}}" class="js-example-basic-single bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" name="state" >
                                                            ...
                                                            <!-- Submit Button -->
                                                            <div class="flex items-center space-x-4">
                                                                <button id='{{$jadwal->id}}-update' type="submit" class="text-blue-500 bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-8 py-2.5 text-center border border-blue-500 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                                    Selesai
                                                                </button>
                                                            </div>
                                                        </div>
                                        ...
                                        <div id="{{$jadwal->id}}-selectRoomModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden bg-gray-500 bg-opacity-25 fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                                    ...
                                                    @php
                                                    $availableRooms = session('availableRooms', collect());
                                                    @endphp

                                                    @if($availableRooms->isEmpty())
                                                    <p class="text-red-500">Tidak ada ruangan yang tersedia.</p>
                                                    @endif
                                        ...
                                        {{-- <form action="{{ route('log_ruangan.store') }}" method="POST">
                                            @csrf
                                            <!-- Your form fields for dosen_id, matakuliah_id, etc. -->
                                            <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="matakuliah_id" name="matakuliah_id" required>
                                            </div>
                                            <!-- Other fields for nama_modul, hari, jam_mulai, etc. -->
                                            <button type="submit" class="btn btn-primary">Simpan Pemetaan</button>
                                        </form>

                                        @if(isset($availableRooms) && count($availableRooms) > 0)
                                            <!-- Modal to select room -->
                                                ...
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="roomSelectionModalLabel">Pilih Ruangan</h5>
                                            ...
                                            <script>
                                                // Show the modal when available rooms exist
                                                var myModal = new bootstrap.Modal(document.getElementById('roomSelectionModal'), {
                                                    keyboard: false
                                                });
                                                myModal.show();
                                            </script>
                                        ...
                                        {{-- <script>
                                            ...
                                            document.addEventListener("DOMContentLoaded", function() {
                                                // Trigger modal opening for Pemetaan
                                                document.getElementById('{{$jadwal->pemetaan->id}}-selectRoomModal').click();
                                            });
                                        </script> --}}
                                @endforeach
                            @endif
